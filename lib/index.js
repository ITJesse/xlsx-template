            ...
            row.findall("c").forEach(function(cell) {
                ...
                if(cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(function(placeholder) {
                        ...
                        if (placeholder.type === "table" && substitution instanceof Array) {
                            ...
                            newCellsInserted = self.substituteTable(
                                row, newTableRows,
                                cells, cell,
                                namedTables, substitution, placeholder.key, placeholder, drawing
                            );
        ...
        if(substitution.length === 0) {
            ...
            substitution.forEach(function(element, idx) {
                ...
                if(idx === 0) { // insert in the row where the placeholders are
                    ...
                    } else if(placeholder.subType=='image'){
                    ...
                    if((idx - 1) < newTableRows.length) {
                        ...
                        row.getchildren().filter(x => x.attrib.r < originalCell.attrib.r).
                        map(x => {
                                let newCell = self.cloneElement(x);
                                newCell.attrib.r = self.joinRef({
                                    row: newRow.attrib.r,
                                    col: self.splitRef(x.attrib.r).col
                                });
                    ...
                    if(value instanceof Array) {
                        ...
                        self.insertCellValue(newCell, value, placeholder);
